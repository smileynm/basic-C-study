...
int main(void) {
	...
	int number = 2;									// 시작 수 : 2
	int flag = 2;									// flag 변수 선언

	while (cnt < 10) {								// cnt가 10이 될때까지 반복 실행
		for (int i = 2; i < number; i++) {
			if (number % i == 0) {						// i 값에 나누어 떨어지면 소수가 아니므로
				flag = 2;						// flag를 초기화 한 후
				continue;						// 다음 수로 continue
			}
			else {								// 만약 나누어 떨어지지 않는다면
				flag++;							// flag 값을 1 올림
			}
			if (flag >= number) {						// flag가 number값과 동일할때까지 나누어 떨어지지 않는다면
				prime[cnt++] = number;					// 그 수가 prime number이기에 배열에 저장
			}
		}
		number++;								// 다음 수로 이동
		flag = 2;								// flag 초기화
	...
	for (int i = 0; i < 10; i++) {
		printf("%d ", prime[i]);						// 배열에 저장되어있는 소수 출력
	}
